...
class Bot(sc2.BotAI):
    ...
    async def on_step(self, iteration: int):       
        ...
        if self.time - self.evoked.get((self.cc.tag, 'train'), 0) > 1.0: 
            if self.vespene > 60 and not UnitTypeId.BANSHEE in self.build_order and self.build_banshee :
                self.build_order.append(UnitTypeId.BANSHEE)
                self.evoked['create'] = self.time

            elif self.vespene > 35 and not UnitTypeId.VIKINGFIGHTER in self.build_order and not self.build_banshee :
        ...
        for unit in self.units.not_structure :
            ...
            if not unit.type_id in [UnitTypeId.RAVEN, UnitTypeId.MULE, UnitTypeId.SIEGETANK, UnitTypeId.SIEGETANKSIEGED, UnitTypeId.VIKINGFIGHTER]:
